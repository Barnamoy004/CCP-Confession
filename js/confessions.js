...
function totalReactions(c) {
  return (c.heart || 0) + (c.laugh || 0) + (c.sad || 0) + (c.eyes || 0) + (c.skull || 0);
}
...
function render(items) {
  ...
  items.forEach(c => {
    ...
    article.className = "confession" + (c.pinned ? " pinned" : "");
    article.innerHTML = `
    ...
    article.querySelectorAll(".reaction").forEach(btn => {
      btn.addEventListener("click", () => react(c.id, btn.dataset.type));
    });
...
function reactionButton(c, type, emoji) {
  return `<button class="reaction" data-type="${type}" aria-label="React ${emoji}">${emoji} ${c[type] || 0}</button>`;
}
...
function applyFilters() {
  ...
  if (sort.value === "reacted") {
    items.sort((a,b) => totalReactions(b) - totalReactions(a));
  } else {
...
async function load() {
  if (!window.ccpReady) {
    loading.textContent = "Connect Supabase in js/config.js to load confessions.";
    return;
  }

  const { data, error } = await ccp.from("confessions")
    .select("id,message,category,nickname,created_at,pinned,heart,laugh,sad,eyes,skull")
    .eq("status","approved")
...
async function react(id, type) {
  const key = `ccp-reacted-${id}-${type}`;
  if (localStorage.getItem(key)) return;

  const { error } = await ccp.rpc("add_reaction", { confession_id: id, reaction_type: type });
  if (error) {
    console.error(error);
    return;
  }
...
randomBtn.addEventListener("click", () => {
  ...
  const item = allConfessions[Math.floor(Math.random() * allConfessions.length)];
  render([item]);
  window.scrollTo({top: document.querySelector(".feed").offsetTop - 20, behavior:"smooth"});
});
